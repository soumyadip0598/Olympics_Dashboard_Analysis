...
def most_successful(df, sport):
    ...
    # Merge the counts with the original dataframe to get other details
    merged_df = x.merge(df, on='Name', how='left')[['Name', 'Medals', 'Sport', 'region']].drop_duplicates('Name')
...
def most_successful_countrywise(df, country):
    ...
    # Count the number of medals won by each athlete from the country
    x = temp_df['Name'].value_counts().reset_index().head(10)
    x.columns = ['Name', 'Medals']

    # Merge the counts with the original dataframe to get other details
    merged_df = x.merge(df, on='Name', how='left')[['Name', 'Medals', 'Sport', 'region', 'Event']].drop_duplicates(
        'Name')
